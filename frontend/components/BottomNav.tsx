...
export default function BottomNav() {
  ...
  const menuHantar = {
    ...
    icon: <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
  };

  const menuSemak = {
    name: "Semakan",
    path: "/pengguna/semak-cadangan",
    icon: <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
  };
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] z-[999]">
      <div className="flex justify-around items-center h-16 pb-[env(safe-area-inset-bottom)] px-1">
        ...
        {navLinks.map((link) => {
          ...
          return (
            <Link 
              key={link.name} 
              href={link.path} 
              className={`flex flex-col items-center justify-center w-full h-full space-y-0.5 transition-colors duration-200 ${isActive ? 'text-[#003B73]' : 'text-slate-400 hover:text-slate-600'}`}
            >
              <div className={`${isActive ? 'bg-blue-50' : 'bg-transparent'} p-1.5 rounded-xl transition-all`}>
                {link.icon}
              </div>
              <span className="text-[9px] sm:text-[10px] font-bold tracking-tight text-center px-1 leading-tight">
                {link.name}
              </span>
            </Link>
          );
